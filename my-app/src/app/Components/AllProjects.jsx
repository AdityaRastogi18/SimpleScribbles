  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 pt-8 gap-x-6 gap-y-10">
      {projects.map((project, index) => (
        <HoverReadMore key={project.name} link={`/projects/${[project._id]}`} icon={faLink} text={"View Full Project"}>
          <div className="flex flex-col my-7 md:my-0 gap-5">
            ...
            <div className="flex flex-col gap-6">
              <h4 className="text-2xl lg:text-3xl font-semibold text-black dark:text-teal-300">{project.name}</h4>
              <p className="line-clamp-2 lg:text-xl leading-6">{project.description}</p>
